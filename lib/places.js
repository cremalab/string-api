...
function autoComplete(term, options = {}) {
  const userLocation = options.userLocation ? options.userLocation.coords : {}
  const location = `${userLocation.latitude},${userLocation.longitude}`
  return new Promise((resolve, reject) => {
    places.autocomplete({
      input: term, types: 'establishment', location: location
    }, (err, response) => {
      if (response.body.status != 'OK') console.log('WARNING:', response.body)
      if (err) { return reject(err) }
...
function fetchFromGoogle(placeid) {
  return new Promise((resolve, reject) => {
    if (process.env.NODE_ENV == 'test') {
      resolve(require('../test/routes/api/responses/gmaps').details)
    }
