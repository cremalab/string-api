...
const register = (app) => {
  ...
  io.on('connection', function(socket){
    ...
    socket.on('userMessage', (message) => {
      console.log('MESSAGE:')
      console.log(message)
      if (ch.wantsToCancel(message.text)) {
        return handleUserResponse({action: 'string:start'})
      }
...
const handleUserResponse = (response) => {
  ...
  return actions.handle(response.action, response, chatInterface).then((res) => {
    ...
    let message = res
    if (typeof res == 'object') { message = res.message }
    connectedSocket.emit(
      'botMessage', ch.respondWith({
        text: message, position: 'center', sentiment: 'sad', params: response.params
      })
    )
  })
}
...
const handleBotResponse = (response) => {
  ...
  if ( response.status.code != 200) {
    ...
    return connectedSocket.emit('botMessage',
      ch.respondWith({
        text: 'Something went terribly wrong', position: 'center',
        sentiment: 'sad', params: response.params
      })
    )
  }
