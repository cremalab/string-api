...
const suggestActivity = (builder, payload, chatInterface) => {
  return ActivitySuggester.suggest({
    ...
    if (err && err.noActivities) {
      ...
      return sleep(1000).then(() => promptActivityInput(payload))
    }
  })
}

const findAndSuggestLocation = (payload, criteria, chatInterface) => {
  criteria = Object.assign(criteria, payload.userLocation)
  return LocationSearcher.findOne(criteria).then((location) => {
    ...
    chatInterface.send({text: err})
    return sleep(1500).then(() => Promise.resolve(actions['string:continue']()))
  })
}
