...
const getBuilder = (userId) => {
  return StringBuilder.model.find(
    {user: userId}).sort('-created_at').exec().then((r) => {
      return r[0]
    })
}
...
const rejectSuggestion = (payload, interface) => {
  return getBuilder(payload.userId).then((builder) => {
    const attrs = Object.assign(payload.params, {rejected: builder.last_location})
    return updateBuilder(payload, attrs)
  }).then((builder) => {
...
const acceptSuggestion = (payload, interface) => {
  return getBuilder(payload.userId).then((builder) => {
    interface.send(`strings.type:${builder.activity_category}.successful_suggestion`)
    return suggestActivity(builder, payload)
  })
}
...
const suggestActivity = (builder, payload) => {
  return ActivitySuggester.suggest({
    ...
    return ch.respondWith({
      ...
      responseOptions: {
        items: [
          {text: 'I did it!', paramName: 'accepted', paramValue: true},
          {text: 'Suggest something else', paramName: 'accepted', paramValue: true},
          {text: 'My own', paramName: 'accepted', paramValue: 'custom'}
        ],
        multi: false
      },
  ...
  return LocationSearcher.findOne(criteria).then((location) => {
    ...
    return updateBuilder(payload, {
      ...
      let res = ch.respondWith({
        ...
        responseOptions: {
          items: [
            {text: 'Sounds good', paramName: 'accepted', paramValue: true},
            {text: 'No thanks', paramName: 'accepted', paramValue: false}
          ],
          multi: false
        },
