...
const actions = {
  ...
  'string:set_activity_type': (payload, chatInterface) => {
    try {
      Joi.assert(payload, stringActionsSchema.set_activity_type)
    } catch(err) {
  ...
  'string:set_party_type': (payload, chatInterface) => {
    try {
      Joi.assert(payload, stringActionsSchema.set_party_type)
    } catch(err) {
      return Promise.reject(err)
    }
  ...
  'string:respond_to_activity_suggestion': (payload) => {
  ...
  'string:create_activity': (payload) => {
    try {
      Joi.assert(payload, stringActionsSchema.create_activity)
    } catch(err) {
      return Promise.reject(err)
    }
    let party_type
...
const acceptActivitySuggestion = (payload) => {
  try {
    Joi.assert(payload, stringActionsSchema.respond_to_activity_suggestion)
  } catch(err) {
    return Promise.reject(err)
  }
  return getBuilder(payload.currentUser._id).then((builder) => {
