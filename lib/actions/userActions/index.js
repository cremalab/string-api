...
const actions = {
  ...
  'user:set_geo': (payload, chatInterface) => {
    ...
    // would love to call chatActions.handle on this
  ...
  'user:set_name': (payload, chatInterface) => {
    return ch.checkSchema(payload, 'user:set_name').then(() => {
      return keystone.list('User').model.findOneAndUpdate(
        ...
        return sleep(1000).then(() => {
          return chatInterface.send({
            text: i18n.t(`users.registration.greeting2`, {name: payload.text})
          })
        })
        .then(() => sleep(2000))
        .then(() => {
  ...
  'user:set_preferences': (payload, chatInterface) => {
    return sleep(2000).then(() => setPreferences(payload,chatInterface))
  },
